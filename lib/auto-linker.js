    ...
    for (const keyword of article.keywords) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (regex.test(parts[i])) {
          parts[i] = parts[i].replace(regex, `<a href="/insights/${article.slug}" class="text-[#2997ff] hover:text-[#0066cc] underline decoration-[#2997ff]/30 hover:decoration-[#2997ff] transition">$&</a>`);
        }
      }
      result = parts.join('');
    }
