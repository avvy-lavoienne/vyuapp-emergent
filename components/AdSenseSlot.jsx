...
export default function AdSenseSlot({ slot, format = 'auto', responsive = true, label = 'Advertisement', style }) {
  ...
  if (!clientId || !slot) {
    return (
      <div className="my-10">
        <div className="rounded-xl border border-dashed border-[#d2d2d7] bg-[#F8F7F4] flex flex-col items-center justify-center text-center px-6 py-12" style={style}>
          <p className="font-mono text-xs text-[#6e6e73] uppercase tracking-[0.15em]">// {label}</p>
  ...
  return (
    <div className="my-10">
      <p className="font-mono text-xs text-[#6e6e73] uppercase tracking-[0.15em] text-center mb-3">// {label}</p>
      <ins
        ref={ref}
