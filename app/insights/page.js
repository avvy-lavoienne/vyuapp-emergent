...
function ArticleCard({ a, featured = false, index = 0 }) {
  return (
    <Link
      ...
      className={`rounded-2xl border border-[#d2d2d7] dark:border-[#333336] bg-white dark:bg-[#1d1d1f] overflow-hidden flex flex-col group transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D1D0C9] dark:hover:border-[#3A3A3D] ${featured ? 'lg:col-span-2' : ''}`}
    >
      <div className={`relative ${featured ? 'h-72' : 'h-52'} overflow-hidden bg-[#F4F3EE] dark:bg-[#333336]`}>
        <Image
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium text-[#6e6e73] dark:text-[#86868b] bg-white/80 dark:bg-[#1d1d1f]/80 border border-[#d2d2d7] dark:border-[#333336]">
            {a.category}
          </span>
        </div>
      </div>
      <div className="p-6 flex flex-col flex-1">
        <div className="flex items-center gap-3 text-xs text-[#6e6e73] dark:text-[#8A8A88] mb-3">
        ...
        <h3 className={`font-sans font-semibold text-[#1d1d1f] dark:text-[#f5f5f7] leading-snug ${featured ? 'text-2xl' : 'text-lg'} group-hover:text-[#2997ff] transition-colors tracking-[-0.01em]`}>
          {a.title}
        </h3>
        <p className="mt-3 text-sm text-[#4A4A48] dark:text-[#86868b] leading-relaxed line-clamp-3">{a.excerpt}</p>
        <div className="mt-5 flex flex-wrap gap-1.5">
          {(a.tags || []).slice(0, 3).map(t => (
            <span key={t} className="text-[10px] font-mono tracking-wider text-[#737370] dark:text-[#8A8A88] uppercase">#{t}</span>
          ))}
        </div>
        <div className="mt-auto pt-5">
          <span className="inline-flex items-center gap-2 text-sm text-[#2997ff] font-medium group-hover:gap-3 transition-all">Baca artikel <ArrowRight className="w-4 h-4" /></span>
        </div>
      </div>
    </Link>
  );
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          {articles.length === 0 ? (
            <div className="p-12 rounded-2xl border border-[#d2d2d7] dark:border-[#333336] bg-white dark:bg-[#1d1d1f] text-center">
              {q ? (
                <>
                  <p className="font-mono text-xs text-[#6e6e73] dark:text-[#8A8A88] uppercase tracking-[0.15em]">// Tidak ditemukan</p>
                ...
                <>
                  <p className="font-mono text-xs text-[#6e6e73] dark:text-[#8A8A88] uppercase tracking-[0.15em]">// Belum ada artikel</p>
                  <p className="mt-3 text-sm text-[#4A4A48] dark:text-[#86868b]">
                    {(category || tags)
