...
function ArticleCard({ a, featured = false, index = 0 }) {
  return (
    <Link
      ...
      className={`rounded-2xl border border-[#E5E4E0] dark:border-[#2A2A2D] bg-white dark:bg-[#1A1A1D] overflow-hidden flex flex-col group transition-all duration-200 hover:-translate-y-0.5 hover:border-[#D1D0C9] dark:hover:border-[#3A3A3D] ${featured ? 'lg:col-span-2' : ''}`}
    >
      <div className={`relative ${featured ? 'h-72' : 'h-52'} overflow-hidden bg-[#F4F3EE] dark:bg-[#2A2A2D]`}>
        <Image
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium text-[#6B6B68] dark:text-[#B0B0B0] bg-white/80 dark:bg-[#1A1A1D]/80 border border-[#E5E4E0] dark:border-[#2A2A2D]">
            {a.category}
          </span>
        </div>
      </div>
      <div className="p-6 flex flex-col flex-1">
        <div className="flex items-center gap-3 text-xs text-[#636360] dark:text-[#8A8A88] mb-3">
        ...
        <h3 className={`font-sans font-semibold text-[#141413] dark:text-[#F0F0F0] leading-snug ${featured ? 'text-2xl' : 'text-lg'} group-hover:text-[#6D5BA0] transition-colors tracking-[-0.01em]`}>
          {a.title}
        </h3>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          {articles.length === 0 ? (
            <div className="p-12 rounded-2xl border border-[#E5E4E0] dark:border-[#2A2A2D] bg-white dark:bg-[#1A1A1D] text-center">
              {q ? (
                <>
                  <p className="font-mono text-xs text-[#636360] dark:text-[#8A8A88] uppercase tracking-[0.15em]">// Tidak ditemukan</p>
                ...
                <>
                  <p className="font-mono text-xs text-[#636360] dark:text-[#8A8A88] uppercase tracking-[0.15em]">// Belum ada artikel</p>
                  <p className="mt-3 text-sm text-[#4A4A48] dark:text-[#B0B0B0]">
                    {(category || tags)
            ...
            <>
              {q && (
                <p className="mb-5 text-sm text-[#636360] dark:text-[#8A8A88]">
